...
internal sealed class ServiceGroupPromotionImageUpsertRequestValidator : AbstractValidator<ServiceGroupPromotionImageUpsertRequest>
{
    public ServiceGroupPromotionImageUpsertRequestValidator()
    {
        RuleFor(x => x.ContentType).NotEmpty().MaximumLength(GroupPromotionImageConstraints.ContentTypeMaxLength).IsImageContentType();
        ...
        RuleFor(x => x.CustomFileNameWithoutExtension).NotEmptyOrWhiteSpace().MaximumLength(GroupPromotionImageFileDataConstraints.FileNameMaxLength);
