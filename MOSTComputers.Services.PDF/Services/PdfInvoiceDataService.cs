...
internal sealed class PdfInvoiceDataService : IPdfInvoiceDataService
{
    ...
    public async Task<InvoiceData> GetPdfInvoiceDataFromInvoiceAsync(Invoice invoice)
    {
        ...
        {
            firmData = await _firmDataRepository.GetByIdAsync(invoice.FirmId.Value);
        }
        ...
        return new()
        {
            ...
            Purchases = invoice.InvoiceItems is not null ? GetPdfInvoicePurchaseDataFromInvoice(invoice.InvoiceItems, vatPercentageFraction, invoiceCurrency) : new(),
    ...
    private List<PurchaseInvoiceData> GetPdfInvoicePurchaseDataFromInvoice(IEnumerable<InvoiceItem> invoiceItems, decimal vatPercentage, Currency currency)
    {
        List<PurchaseInvoiceData> output = new();

        foreach (InvoiceItem invoiceItem in invoiceItems)
        {
            output.Add(GetPdfInvoicePurchaseDataFromInvoiceData(invoiceItem, vatPercentage, currency));
        }
    ...
    private PurchaseInvoiceData GetPdfInvoicePurchaseDataFromInvoiceData(
        InvoiceItem invoiceItem, decimal vatPercentage, Currency currency)
    {
