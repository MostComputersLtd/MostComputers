...
internal sealed class InvoiceXmlService : IInvoiceXmlService
{
    private const string _invalidXmlDefaultMessage = "Something went wrong";

    public async Task TrySerializeXmlAsync(Stream outputStream, InvoiceXmlFullData xmlData)
    {
    ...
    public async Task<OneOf<string, InvalidXmlResult>> TrySerializeXmlAsync(InvoiceXmlFullData xmlData)
    {
